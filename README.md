...
Open your terminal or command prompt and run the following command to create a Conda environment named `cora` ( or any name you prefer) with Python 3.10:
...
The next step is to install all the dependencies required to execute the experiments. First, install the [DGL library](https://www.dgl.ai/pages/start.html) according to the GPU availability and Pytorch Version(2.1.x).

 To install the rest of the required dependencies, run:
...
python main.py
```
This will execute the node classification on the CORA dataset using 10 10-fold data split. The mean validation accuracy  with standard deviation over k-folds will be printed. Finally, the accuracy on the entire dataset will be printed as well.
...
There is also a provision to set the number of folds for data split, which is kept at 10 as the default value. To run the experiments with different numbers of folds, run with the command:
```shell
